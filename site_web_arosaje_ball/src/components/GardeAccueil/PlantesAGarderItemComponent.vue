<template>
  ...
  <v-card
          ...
          max-width="100%"
  >
    <v-row justify="center"  v-if=" garde !== null && garde.plante.proprietaire.id === user.id">
  ...
  <v-row>
    ...
    <v-col>
      <v-card class="ma-10" prepend-icon="mdi-flower-tulip">
        ...
        <v-btn v-if="plante.proprietaire.id !== user.id && garde.statut.id !== 1" @click="dialog = true" class="ma-5">GARDER</v-btn>
      </v-card>
    </v-col>
  </v-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    getCommentaires(){
      axios.get("https://arosaje-back.jadedomasvasserot.com/commentaire/all/byBotaniste/"+ this.idGarde,
        {
          withCredentials: false,
          headers: {
            'Authorization': 'Bearer ' +this.$store.state.token,
    ...
    async getFormValues(id, newComp) {
      await axios.post(
        'https://arosaje-back.jadedomasvasserot.com/commentaire/add',
        {
          titre: this.titre,
    ...
    getPlanteId() {
      axios.get("https://arosaje-back.jadedomasvasserot.com/garde-plante/id/" + this.idGarde,
        {
          withCredentials: false,
          headers: {
            'Authorization': 'Bearer ' + this.$store.state.token,
        ...
        .then(rep => {
            if (rep.data) {
              ...
              axios.get(`https://arosaje-back.jadedomasvasserot.com/photo-plante/all/idPlante/${rep.data.plante.id}`,
                {
                  withCredentials: false,
                  headers: {
                    'Authorization': 'Bearer ' + this.$store.state.token,
    ...
    addGarde(id, newComp) {
      axios.put("https://arosaje-back.jadedomasvasserot.com/garde-plante/"+this.garde.id+"/update/gardien/"+this.$store.state.user+"/status",
        {
          withCredentials: false,
          headers: {
            'Authorization': 'Bearer ' + this.$store.state.token,
    ...
    getVisiteByGarde() {
      axios.get("https://arosaje-back.jadedomasvasserot.com/visite-plante/all/byGarde/" +this.idGarde,
        {
          withCredentials: false,
          headers: {
            'Authorization': 'Bearer ' + this.$store.state.token,
